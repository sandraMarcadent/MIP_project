...
class Data_loader():
    ...
    def load_data(self):
        ...
        with h5py.File(self.data_path, 'r') as hdf:
            ...
            if (self.max_images_to_open > 0):
                ...
                _y = hdf['y_modelName'][:self.max_images_to_open]
            else:
                X = hdf['X'][:]
                _y = hdf['y_modelName'][:]
    ...
    def format_to_classes(self):                      
            ...
            self.processed_a = self.processed_X[np.where(self.labels==b'Fluorospot_Compact_FD')]
            self.processed_b = self.processed_X[np.where(self.labels==b'DigitalDiagnost')]
            print('Data is devided into class a: Fluorospot_Compact_FD and class b: DigitalDiagnost') 
    ...
    def split_data(self): 
            ...
            if self.testing:
                ...
                # CLASS A SPLIT
                lim_val = int(self.processed_a.shape[0]*self.prop_val)

                self.a_train = self.processed_a[:-lim_val,:,:]
                self.a_val = self.processed_a[-lim_val:,:,:]
                self.a_test = self.a_val
      

                # CLASS B SPLIT
                lim_val = int(self.processed_b.shape[0]*self.prop_val)

                self.b_train = self.processed_b[:-lim_val,:,:]
                self.b_val = self.processed_b[-lim_val:,:,:]
                self.b_test = self.b_val

                #FULL DATA SPLIT
                lim_val = int(self.processed_X.shape[0]*(self.prop_val))

                self.x_train = self.processed_X[:-lim_val,:,:]
                self.x_val = self.processed_X[-lim_val:,:,:]
                self.x_test = self.x_val

                self.y_train = self.y[:-lim_val]
                self.y_val = self.y[-lim_val:]
