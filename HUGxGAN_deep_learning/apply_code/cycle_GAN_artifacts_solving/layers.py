...
def general_resizeconv2d(inputconv, target_height, target_width, o_d=64, f_h=7, f_w=7,
                     stddev=0.02, padding="VALID", name="resizeconv2d",
                     do_norm=True, do_relu=True, relufactor=0):
    
    with tf.variable_scope(name):

        
        res = tf.image.resize_images(inputconv, [int(target_height), int(target_width)], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        
        conv = tf.contrib.layers.conv2d(
            res, o_d, f_w, 1, padding,
            activation_fn=None,
            weights_initializer=tf.truncated_normal_initializer(
                stddev=stddev
            ),
