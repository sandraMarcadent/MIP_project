...
def wasserstein_loss_discriminator(prob_real_is_real, prob_fake_is_real):
    ...
    //////////////
    """
    return tf.reduce_mean(prob_fake_is_real-prob_real_is_real)

def gradient_penalty(hat_input, hat_output):
    '''
    //////////////
    '''
    gradients = tf.gradients(hat_output, [hat_input])
    
    return tf.square(tf.norm(gradients[0], ord=2)-1)
