...
def output_to_img(sess, ndarray, nchannels):
            ...
            if nchannels == 1: 
                img = sess.run(tf.image.grayscale_to_rgb(ndarray*255)).astype(np.uint24)

            else:
                img = (ndarray*255).astype(np.uint24)

            return img
                    
################################################################################################

def from_label_to_int(y):
    y_names = set(y)
    ...
    for idx, value in enumerate(y_names):
        ...
        print(idx, value)
        print('len :', len(y_int[np.where(y == value)]))
